...
class Args:
    ...
    """exploration method: none or icm"""

    icm_coef: float = 0.01
    """coefficient for intrinsic curiosity reward"""

    icm_lr: float = 1e-4
    """learning rate for ICM"""

    icm_feature_dim: int = 128
    """feature dimension for ICM encoder"""

    icm_beta: float = 0.2
    """weight on forward loss in ICM loss"""
...
class ICM(nn.Module):
    def __init__(self, obs_dim, action_dim, feature_dim=128):
        super().__init__()
    ...
    def forward(self, obs, next_obs, actions):
        ...
        next_phi = self.encoder(next_obs)
        ...
        # forward prediction
        action_onehot = F.one_hot(actions.long(), num_classes=self.action_dim).float()
        forward_input = torch.cat([phi, action_onehot], dim=1)
        pred_next_phi = self.forward_model(forward_input)

        return phi, next_phi, pred_next_phi, pred_action_logits

    def intrinsic_reward(self, obs, next_obs, actions):
        """
        Returns curiosity reward for each transition.
        Shape: [batch]
        """
        with torch.no_grad():
            phi, next_phi, pred_next_phi, _ = self.forward(obs, next_obs, actions)

            # per-sample forward prediction error
            intrinsic_reward = 0.5 * ((pred_next_phi - next_phi) ** 2).mean(dim=1)

        return intrinsic_reward

    def loss(self, obs, next_obs, actions, beta=0.2):
        phi, next_phi, pred_next_phi, pred_action_logits = self.forward(
            obs, next_obs, actions
        )

        inverse_loss = F.cross_entropy(pred_action_logits, actions.long())

        # detach next_phi so the forward model predicts a fixed feature target
        forward_loss = 0.5 * ((pred_next_phi - next_phi.detach()) ** 2).mean()
...
if __name__ == "__main__":
    ...
    # ICM modification
    next_obs_buffer = torch.zeros((args.num_steps, args.num_envs) + envs.single_observation_space.shape).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            if args.exploration == "icm":
                obs_flat = obs[step].reshape(args.num_envs, -1)
                next_obs_flat = next_obs_buffer[step].reshape(args.num_envs, -1)
                action_flat = actions[step].reshape(args.num_envs)

                intrinsic_rewards[step] = icm.intrinsic_reward(
                    obs_flat,
                    next_obs_flat,
                    action_flat,
                )
            else:
                intrinsic_rewards[step] = 0.0
        ...
        with torch.no_grad():
            ...
            for t in reversed(range(args.num_steps)):
                ...
                delta = total_rewards[t] + args.gamma * nextvalues * nextnonterminal - values[t]
